...
def collect_cleanup_candidates(now: Optional[float] = None) -> List[Dict[str, Any]]:
    ...
    for category, info in _CATEGORY_INFO.items():
        items = sorted(
...
def _fallback_ai(
    snapshot: Mapping[str, Any], candidates: List[Dict[str, Any]]
) -> _AIDiagnosis:
    ...
    if pressure >= 80:
        ...
        why.append(
            f"메모리 압박 점수가 {pressure:.0f}점으로 다소 높습니다."
        )
    ...
    if not why:
        why.append("현재 수치만으로는 뚜렷한 디스크나 메모리 병목이 보이지 않습니다.")

    recommendations = [
        _AIRecommendation(
            category=candidate["category"],
            reason=_CATEGORY_INFO[candidate["category"]]["reason"],
        )
        ...
        recommendations.append(
            {
                **candidate,
                ...
                or _CATEGORY_INFO[category]["reason"],
                "action": _CATEGORY_INFO[category]["action"],
            }
        )
...
def diagnose(
    ...
    """
    snapshot = dict(metrics_snapshot) if metrics_snapshot is not None else collect_metrics()
    candidates = collect_cleanup_candidates()
    api_key = _load_api_key()
    if not api_key:
        fallback = _fallback_ai(snapshot, candidates)
        return _assemble_result(
            fallback, candidates, "fallback", fallback_reason="missing_api_key"
        )
    ...
    try:
        ...
        return _assemble_result(response.output_parsed, candidates, "openai")
    except Exception:
        fallback = _fallback_ai(snapshot, candidates)
        return _assemble_result(
            fallback, candidates, "fallback", fallback_reason="api_error"
        )
